...
{
public:
    void Free(void* Original) { static void (*FreeOG)(void* Original) = decltype(FreeOG)(uintptr_t(GetModuleHandle(0)) + 0xDE9140); return FreeOG(Original); };
    void* Realloc(void* Original, __int64 Count, uint32_t Alignment) { static void* (*ReallocOG)(void* Original, __int64 Count, uint32_t Alignment) = decltype(ReallocOG)(uintptr_t(GetModuleHandle(0)) + 0x3E54B6C); return ReallocOG(Original, Count, Alignment); };
};
...
{	
	...
	public:
        ...
        /* Reserves space for Number more elements.Previous data is kept. */
		inline void Reserve(int Number, int Size = sizeof(ArrayElementType))
		{
			Data = static_cast<ArrayElementType*>(FMemory().Realloc(Data, (MaxElements = NumElements + Number) * Size, 0));
		}

        /* Frees the array and resets it to default state. */
		inline void Free()
		{
			if (Data)
				FMemory().Free(Data);
		...
		inline bool Add(const ArrayElementType& Element, int Size = sizeof(ArrayElementType))
		{
			/*if (GetSlack() <= 0)
				return false;*/
	...
	*/
	...
	public:
		...
		bool Contains(const SetElementType& Key) const
		{
			for (int32 Index = 0; Index < Elements.Max(); ++Index)
			{
				if (!Elements.IsValidIndex(Index))
					continue;

				if (Elements[Index].Value == Key)
					return true;
			}
